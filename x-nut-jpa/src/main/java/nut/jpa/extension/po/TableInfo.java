...
import nut.jpa.extension.StrategyConfig;
...
public class TableInfo {
    ...
    protected void setConvert(StrategyConfig strategyConfig) {
        if (strategyConfig.containsTablePrefix(name)) {
            ...
            if (StrategyConfig.DB_COLUMN_UNDERLINE) {
                // 包含大写处理
                if (DbStringUtils.containsUpperCase(name)) {
                    this.convert = true;
                }
    ...
    public void setFields(List<TableField> fields) {
        if (DbCollectionUtils.isNotEmpty(fields)) {
            this.fields = fields;
            // 收集导入包信息
            ...
            for (TableField field : fields) {
                ...
                if (field.isKeyFlag()) {
                    ...
                    if (field.isConvert() || field.isKeyIdentityFlag()) {
                        pkgSet.add("com.x.nut.annotations.TableId");
                    }
                    // 自增
                    if (field.isKeyIdentityFlag()) {
                        pkgSet.add("com.x.nut.enums.IdType");
                    }
                } else if (field.isConvert()) {
                    // 普通字段
                    pkgSet.add("com.x.nut.annotations.TableField");
                }
                if (null != field.getFill()) {
                    // 填充字段
                    pkgSet.add("com.x.nut.annotations.TableField");
                    pkgSet.add("com.x.nut.enums.FieldFill");
                }
            }
            if (!pkgSet.isEmpty()) {
    ...
    public String getFieldNames() {
        if (DbStringUtils.isEmpty(fieldNames)) {
