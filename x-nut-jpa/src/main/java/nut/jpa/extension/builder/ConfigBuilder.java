package nut.jpa.extension.builder;

import nut.jpa.extension.*;
...
import nut.jpa.extension.rules.QuerySQL;
...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        if (DbStringUtils.isNotEmpty(template.getEntity())) {
            pathInfo.put(ConstVal.ENTITY_PATH, joinPath(outputDir, packageInfo.get(ConstVal.ENTITY)));
        }

        if (DbStringUtils.isNotEmpty(template.getQuery())) {
            pathInfo.put(ConstVal.QUERY_PATH, joinPath(outputDir, packageInfo.get(ConstVal.QUERY)));
        }
        ...
        if (DbStringUtils.isNotEmpty(template.getServiceImpl())) {
            pathInfo.put(ConstVal.SERVICEIMPL_PATH, joinPath(outputDir, packageInfo.get(ConstVal.SERVICEIMPL)));
        }
        if (DbStringUtils.isNotEmpty(template.getController())) {
...
//        }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            if (DbStringUtils.isNotEmpty(globalConfig.getQueryName())) {
                tableInfo.setQueryName(String.format(globalConfig.getQueryName(), tableInfo.getEntityName()));
            } else {
            ...
            if (DbStringUtils.isNotEmpty(globalConfig.getServiceImplName())) {
            ...
            if (DbStringUtils.isNotEmpty(globalConfig.getControllerName())) {
    ...
    private List<TableInfo> getTablesInfo(StrategyConfig config) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (DbStringUtils.isNotEmpty(tableName)) {
    ...
    private TableInfo convertTableFields(TableInfo tableInfo, NamingStrategy strategy) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                boolean isId = DbStringUtils.isNotEmpty(key) && key.toUpperCase().equals("PRI");
                // 处理ID
                if (isId && !haveId) {
    ...
    private String joinPath(String parentDir, String packageName) {
        if (DbStringUtils.isEmpty(parentDir)) {
            parentDir = System.getProperty(ConstVal.JAVA_TMPDIR);
        }
        if (!DbStringUtils.endsWith(parentDir, File.separator)) {
    ...
    private String joinPackage(String parent, String subPackage) {
        if (DbStringUtils.isEmpty(parent)) {
            return subPackage;
        }
