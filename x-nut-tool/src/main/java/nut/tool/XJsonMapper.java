 ...
 * 
 * 可以直接使用公共示例XJsonMapper.INSTANCE, 也可以使用不同的builder函数创建实例，封装不同的输出风格,
 * 
 * 不要使用GSON, 在对象稍大时非常缓慢.
 * 
...
public class XJsonMapper {
	...
	public static final XJsonMapper INSTANCE = new XJsonMapper();
	...
	public XJsonMapper(Include include) {
	 ...
	 */
	public static XJsonMapper nonNullMapper() {
		return new XJsonMapper(Include.NON_NULL);
	}
	 ...
	 */
	public static XJsonMapper nonEmptyMapper() {
		return new XJsonMapper(Include.NON_EMPTY);
	}

	/**
	 * 默认的全部输出的Mapper, 区别于INSTANCE，可以做进一步的配置
	 */
	public static XJsonMapper defaultMapper() {
		return new XJsonMapper();
	}
